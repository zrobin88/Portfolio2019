...
class Jumbotron extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            description: ['Strong password generator built in React.js','The A-List is a MERN stack app that helps musicians connect with each and find the right person for the job.','This is a React video app that queries the YouTube API to return a list of videos based upon a search term', 'This is a React app that uses the Last.fm API to return information on any musical artist.', 'A React App that queries the Unsplash API for Images.', 'An app that converts units of measurement.', 'A C-R-U-D application that utilizes mongo db to help users manage boxes and inventory. ', 'This app is a game based on state, event handlers and components in React.js. Each card contains a set score and other data attributes. When the user clicks on a card, the app responds to the users click and alters the app state accordingly.'],
